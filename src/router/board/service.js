...
const getBoardList = async (req, res, next) => {
  ...
  try {
    const result = await client.query(getBoardListSQL, [category, page]);
...
const getBoard = async (req, res, next) => {
  ...
  try {
    // 조회수 증가
    await client.query(increaseViewCountSQL, [board_list_idx]);
    const result = await client.query(getBoardSQL, [board_list_idx]);
...
const postBoard = async (req, res, next) => {
  ...
  const { player_list_idx, board_list_title, board_list_content } = req.body;
...
const putBoard = async (req, res, next) => {
  const { board_list_idx } = req.params;
  const { board_list_title, board_list_content } = req.body;
  ...
  try {
    ...
    await client.query(putBoardSQL, [
      board_list_idx,
      board_list_title,
      board_list_content,
      new_img_url,
...
const boardLike = async (req, res, next) => {
  ...
  try {
    await client.query("BEGIN");
    await client.query(likeIncreaseSQL, [board_list_idx]);
    await client.query(boardLikeSQL, [board_list_idx, player_list_idx]);
...
const boardLikeDelete = async (req, res, next) => {
  ...
  try {
    await client.query("BEGIN");
    await client.query(likeDecreaseSQL, [board_list_idx]);
    await client.query(boardLikeDeleteSQL, [board_list_idx, player_list_idx]);
    await client.query("COMMIT");
    res.status(200).send({});
  } catch (e) {
    await client.query("ROLLBACK");
...
const postComment = async (req, res, next) => {
  const { board_list_idx } = req.params;
  const { player_list_idx, board_comment_content } = req.body;
...
const putComment = async (req, res, next) => {
  ...
  const { board_comment_content } = req.body;
...
const deleteComment = async (req, res, next) => {
  ...
  try {
    await client.query(deleteCommentSQL, [board_comment_idx]);
